...
export async function POST(req:Request){
  try{
    ...
    const {error:campaignError}=await db.from("ad_campaigns").insert({business_id:businessId,product_id:product.id,headline:requestedHeadline||`Advertising request — ${businessName}`,ad_copy:adCopy,destination_url:website||null,start_date:startDate,end_date:endDate,billing_price_cents:priceCents,booked_duration_days:days,pricing_term:pricingTerm,approved:false,paid:false,status:"draft"});if(campaignError)throw campaignError;
