...
export async function POST(req:Request){
  try{
    ...
    if(!businessId){const slug=`${slugify(businessName)}-advertising-lead-${crypto.randomUUID().slice(0,8)}`,hasPublicLocation=b.public_location==="yes";const {data:created,error}=await db.from("businesses").insert({name:businessName,slug,phone:phone||null,website:website||null,email,category:"Other",location_type:hasPublicLocation?"physical":"service_area",listing_visibility:"private",short_description:"Advertising inquiry — not published",description:"Private advertiser record created from a customer advertising request.",active:false}).select("id").single();if(error||!created)throw error||new Error("Unable to create advertiser record");businessId=created.id}
    ...
    const customerRequest=[`Contact: ${contactName}`,`Email: ${email}`,`Phone: ${phone||"Not provided"}`,`Website: ${website||"Not provided"}`,`Public visitor location: ${b.public_location==="yes"?"Yes":"No / online / private property"}`,`Requested service: ${product.name}`,`Requested term: ${days} days · $${(priceCents/100).toFixed(2)}`,"","Customer message:",message].join("\n");
    const acceptedAt=new Date().toISOString();
    const {data:campaign,error:campaignError}=await db.from("ad_campaigns").insert({business_id:businessId,product_id:product.id,headline:requestedHeadline||`Advertising request — ${businessName}`,ad_copy:message,customer_request:customerRequest,contact_name:contactName,contact_email:email,contact_phone:phone||null,destination_url:safeExternalUrl(website)||null,start_date:startDate,end_date:endDate,billing_price_cents:priceCents,booked_duration_days:days,pricing_term:pricingTerm,approved:false,paid:false,status:"draft",agreement_version:ADVERTISING_AGREEMENT_VERSION,agreement_accepted_at:acceptedAt,onboarding_request_id:submissionId}).select("id,created_at,contact_email,contact_name,start_date,booked_duration_days,businesses(name)").single();if(campaignError?.code==="23505")return privateJson({ok:true,emailSent:true,duplicate:true});if(campaignError||!campaign)throw campaignError||new Error("Campaign was not returned after saving");
