...
export default async function Page({params}:{params:Promise<{slug:string}>}){
  ...
  <aside className="h-fit border border-cream/10 bg-white p-6"><h2 className="font-display text-2xl">Listing details</h2><div className="mt-5 space-y-5 text-sm">{b.address&&<p className="flex gap-3"><MapPin className="shrink-0 text-forest" size={20}/>{b.address}</p>}{b.phone&&<p className="flex gap-3"><Phone className="shrink-0 text-forest" size={20}/>{b.phone}</p>}<div className="flex gap-3"><Clock className="mt-0.5 shrink-0 text-forest" size={20}/><div>{b.hours?<><p className="whitespace-pre-line font-medium leading-6">{b.hours}</p>{b.hoursVerifiedAt&&<p className="mt-2 text-xs text-ink/50">Verified {new Date(`${b.hoursVerifiedAt}T12:00:00`).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}</p>}{b.hoursSource&&<a href={b.hoursSource} target="_blank" rel="noreferrer" className="mt-1 inline-block text-xs font-bold text-forest underline">Hours source ↗</a>}</>:<p>Hours are not reliably published — call or check the business website before visiting.</p>}</div></div></div><p className="mt-6 border-t border-cream/10 pt-5 text-xs leading-5 text-ink/55">Hours and services can change seasonally or on holidays. Confirm directly before making a special trip.</p></aside></section></>
}
