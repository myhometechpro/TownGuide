...
export default async function Page({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{inquiry?:string}>}){
  ...
  const {data}=await db.from("businesses").select("id,name,slug,category,short_description,description,address,phone,email,website,image_url,latitude,longitude,location_type,lodging_type,hours_json,featured,sponsored,active").eq("id",id).maybeSingle();
  if(!data)notFound();
