...
export function Header(){return <header className="sticky top-0 z-40 border-b border-cream/10 bg-pine/95 text-cream backdrop-blur">
  <div className="mx-auto flex min-h-18 max-w-7xl items-center justify-between px-4 py-3 lg:px-8">
    ...
    <details className="group relative lg:hidden">
      ...
      <nav aria-label="Mobile primary" className="absolute right-0 top-[calc(100%+12px)] grid w-72 gap-1 border border-cream/15 bg-pine p-3 shadow-2xl">{links.map(([label,href])=><Link className="px-4 py-3 font-semibold hover:bg-white hover:text-pine" href={href} key={label}>{label}</Link>)}<Link href="/contact?interest=Free%20Listing" className="mt-2 bg-cream px-4 py-4 text-center font-black text-pine">Add your business</Link></nav>
    </details>
  </div>
</header>}
