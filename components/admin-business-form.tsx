...
export function AdminBusinessForm({business,inquiryId,action,connected=true,deleteControl}:{business?:Partial<Business>;inquiryId?:string;action:(formData:FormData)=>void|Promise<void>;connected?:boolean;deleteControl?:React.ReactNode}){
  ...
  return <form action={action} className="mt-8 grid gap-5 border border-cream/10 bg-white p-6 md:grid-cols-2">
    ...
    <label className="text-sm font-bold">Listing visibility<select className={field} name="listing_visibility" defaultValue={business?.listingVisibility||"directory"}><option value="directory">Directory listing</option><option value="advertiser_only">Advertiser-only profile</option><option value="private">Private advertiser record</option></select></label>
    ...
    <label className="flex items-center gap-3 text-sm font-bold"><input className="h-5 w-5" type="checkbox" name="active" defaultChecked={business?.active!==false}/>Public profile enabled</label>
    <p className="text-xs text-ink/55 md:col-span-2">Directory listings appear in public browsing. Advertiser-only profiles are reachable from ads but stay out of the directory and map. Private records remain in Admin only.</p>
    <label className={`flex items-center gap-3 border p-4 text-sm font-bold ${business?.displayingQrSign?"border-emerald-400 bg-emerald-900/35 text-emerald-100":"border-cream/20"}`}><input className="h-5 w-5 accent-emerald-500" type="checkbox" name="displaying_qr_sign" defaultChecked={business?.displayingQrSign}/>Displaying QR sign</label>
