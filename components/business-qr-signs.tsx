...
export type QrSign={name:string;slug:string;code:string;trackingCode:string;scans:number};
const safe=(value:string)=>value.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");
export const qrSignFilename=(sign:QrSign)=>`heber-overgaard-sign-${sign.trackingCode.toLowerCase()}-${safe(sign.name)}.png`;
const csvCell=(value:string|number)=>`"${String(value).replaceAll('"','""')}"`;

function crossReferenceCsv(signs:QrSign[]){
  ...
  const rows=signs.map((sign,index)=>[sign.trackingCode,sign.name,`https://visitheberovergaard.com/go/${sign.code}`,"https://visitheberovergaard.com/",qrSignFilename(sign),Math.floor(index/2)+1,index%2===0?"left":"right",sign.scans]);
...
export async function renderQrSign(sign:QrSign){
...
export function BusinessQrSigns({signs}:{signs:QrSign[]}){
  const [busy,setBusy]=useState<string>();
  async function download(sign:QrSign){setBusy(sign.code);try{const url=await renderQrSign(sign),a=document.createElement("a");a.href=url;a.download=qrSignFilename(sign);a.click()}finally{setBusy(undefined)}}
  ...
  async function downloadAll(){setBusy("all");try{const zip=new JSZip();for(const sign of signs){const url=await renderQrSign(sign);zip.file(qrSignFilename(sign),url.split(",")[1],{base64:true})}zip.file("heber-overgaard-qr-sign-cross-reference.csv",crossReferenceCsv(signs));saveBlob(await zip.generateAsync({type:"blob"}),"heber-overgaard-business-qr-signs.zip")}finally{setBusy(undefined)}}
  return <>
