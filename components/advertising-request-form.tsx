...
export function AdvertisingRequestForm({ products, initialPlacement }: { products: AdProduct[]; initialPlacement?: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await response.json() as { emailSent?: boolean; error?: string };
      if (!response.ok) {
        setErrorMessage(result.error || "We couldn't save that request. Please try again.");
        setState("error");
        return;
      }
      setState(result.emailSent ? "sent" : "saved_no_email");
    } catch {
  ...
  return <form onSubmit={submit} className="grid gap-5 border border-cream/10 bg-white p-6 shadow-soft md:grid-cols-2">
    ...
    <label className="text-sm font-bold">Business or property name<input name="business_name" required maxLength={200} className={input}/></label>
    ...
    <label className="text-sm font-bold md:col-span-2">Business website<input name="website" type="text" inputMode="url" maxLength={500} className={input} placeholder="Website, social page, or other link"/></label>
    <fieldset className="border border-forest/20 p-4 md:col-span-2"><legend className="px-2 text-sm font-bold">Can visitors go to a public business location?</legend><div className="mt-2 flex flex-wrap gap-6"><label className="flex items-center gap-2 text-sm"><input type="radio" name="public_location" value="yes"/> Yes</label><label className="flex items-center gap-2 text-sm"><input type="radio" name="public_location" value="no" defaultChecked/> No / online / private property</label></div><p className="mt-2 text-xs text-ink/55">Private homes and vacation rentals do not need to provide or publish an address.</p></fieldset>
    ...
    <div className="md:col-span-2"><button disabled={state === "sending" || products.length === 0} className="min-h-13 bg-pine px-7 py-4 font-bold text-cream disabled:opacity-50">{state === "sending" ? "Sending request…" : "Request advertising"}</button>{state === "error" && <p role="alert" className="mt-3 text-sm font-bold text-red-600">{errorMessage}</p>}</div>
  </form>;
}
